...
export default function LearnerLayout() {
    ...
    const navClass = (path) => {
        ...
        return `flex items-center gap-3 px-6 py-3 transition-colors duration-200 ${active
    ...
    const topNavClass = (path) => {
        const active = isActive(path);
        return `font-bold transition-all ${active
            ...
            <div className="bg-[#f9f9f9] text-[#1a1c1c] selection:bg-[#ffdad9] selection:text-[#400009] font-body min-h-screen flex flex-col">
                ...
                <nav className="fixed top-0 w-full z-50 bg-[#f9f9f9]/80 backdrop-blur-md shadow-sm flex justify-between items-center px-6 py-3 font-headline tracking-tight">
                    <div className="flex items-center gap-8">
                        ...
                        <div className="hidden md:flex items-center gap-6">
                            <Link className={topNavClass('/learn')} to="/learn">Bảng điều khiển</Link>
                            <Link className={topNavClass('/learn/study')} to="/learn/study">Bộ thẻ học</Link>
                            <Link className={topNavClass('/learn/dictionary')} to="/learn/dictionary">Từ điển</Link>
                        </div>
                    </div>
                ...
                <aside className="w-64 fixed left-0 top-0 bg-zinc-50 flex-col h-full py-6 hidden lg:flex pt-20 border-r border-zinc-100 z-40">
                    <div className="px-6 mb-8 flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-xl font-black text-red-800 leading-none truncate w-36" title={user?.displayName || user?.username || 'Minori'}>
                    ...
                    <nav className="flex-1 font-body font-medium flex flex-col gap-1">
                        <Link className={navClass('/learn')} to="/learn">
                            <span className="material-symbols-outlined" style={isActive('/learn') ? { fontVariationSettings: "'FILL' 1" } : {}}>dashboard</span>
                            Bảng điều khiển
                        </Link>
                        <Link className={navClass('/learn/study')} to="/learn/study">
                            <span className="material-symbols-outlined" style={isActive('/learn/study') ? { fontVariationSettings: "'FILL' 1" } : {}}>style</span>
                            Bộ thẻ học
                        </Link>
                        <Link className={navClass('/learn/dictionary')} to="/learn/dictionary">
                            <span className="material-symbols-outlined" style={isActive('/learn/dictionary') ? { fontVariationSettings: "'FILL' 1" } : {}}>translate</span>
                            Từ điển
                        </Link>
                        <Link className={navClass('/learn/leaderboard')} to="/learn/leaderboard">
                            <span className="material-symbols-outlined" style={isActive('/learn/leaderboard') ? { fontVariationSettings: "'FILL' 1" } : {}}>leaderboard</span>
                            Bảng xếp hạng
                        </Link>

                        <div className="mt-12 px-6">
                            <Link to="/learn/study" className="w-full bg-[#e2e2e2] text-[#1a1c1c] py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                    ...
                    <div className="px-6 flex flex-col gap-1">
                        <a className="flex items-center gap-3 text-zinc-500 py-2 hover:text-zinc-800 transition-colors font-medium text-sm" href="#">
                ...
                <footer className="lg:ml-64 bg-zinc-100 border-t border-zinc-200 flex flex-col md:flex-row justify-between items-center p-8 mt-auto font-body text-sm">
                    ...
                    <div className="flex gap-6 mt-4 md:mt-0">
                        <a className="text-zinc-500 hover:text-[#8f0020] transition-colors" href="#">Điều khoản</a>
                        <a className="text-zinc-500 hover:text-[#8f0020] transition-colors" href="#">Chính sách bảo mật</a>
                        <a className="text-zinc-500 hover:text-[#8f0020] transition-colors" href="#">Liên hệ hỗ trợ</a>
                    </div>
                </footer>
                ...
                <nav className="lg:hidden fixed bottom-0 left-0 w-full bg-[#f9f9f9]/90 backdrop-blur-xl border-t border-zinc-200 flex justify-around py-2 z-50 pb-safe">
                    <Link className={`flex flex-col items-center ${isActive('/learn') ? 'text-[#8f0020]' : 'text-zinc-400'}`} to="/learn">
                        <span className="material-symbols-outlined" style={isActive('/learn') ? { fontVariationSettings: "'FILL' 1" } : {}}>dashboard</span>
                        <span className="text-[10px] font-bold uppercase tracking-tighter mt-1">Dashboard</span>
                    </Link>
                    <Link className={`flex flex-col items-center ${isActive('/learn/study') ? 'text-[#8f0020]' : 'text-zinc-400'}`} to="/learn/study">
                        <span className="material-symbols-outlined" style={isActive('/learn/study') ? { fontVariationSettings: "'FILL' 1" } : {}}>style</span>
                        <span className="text-[10px] font-bold uppercase tracking-tighter mt-1">Thẻ học</span>
                    </Link>
                    <Link className={`flex flex-col items-center ${isActive('/learn/dictionary') ? 'text-[#8f0020]' : 'text-zinc-400'}`} to="/learn/dictionary">
                        <span className="material-symbols-outlined" style={isActive('/learn/dictionary') ? { fontVariationSettings: "'FILL' 1" } : {}}>translate</span>
                        <span className="text-[10px] font-bold uppercase tracking-tighter mt-1">Từ điển</span>
                    </Link>
                    <Link className={`flex flex-col items-center ${isActive('/learn/leaderboard') ? 'text-[#8f0020]' : 'text-zinc-400'}`} to="/learn/leaderboard">
                        <span className="material-symbols-outlined" style={isActive('/learn/leaderboard') ? { fontVariationSettings: "'FILL' 1" } : {}}>leaderboard</span>
