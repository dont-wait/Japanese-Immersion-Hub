...
export default function ToastNotification({ message, type = 'info', duration = 4000, onClose }) {
    ...
    return (
        <div
            className={`fixed top-20 right-4 z-[200] flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg ${config.bg} ${config.text} transition-all duration-300 ${visible ? 'animate-fade-in-up opacity-100' : 'opacity-0 translate-y-2'
                }`}
        >
            <Icon size={18} />
            <span className="text-sm font-medium">{message}</span>
            <button onClick={() => { setVisible(false); onClose(); }} className="ml-2 p-0.5 rounded-full hover:bg-white/20">
                <X size={14} />
            </button>
        </div>
    );
