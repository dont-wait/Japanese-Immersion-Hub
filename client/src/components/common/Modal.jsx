...
export default function Modal({ isOpen, onClose, title, children, size = 'md', theme = 'auto' }) {
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 ${isLight ? 'light' : ''}`}>
            {/* Backdrop */}
            <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} />

            {/* Modal */}
            <div className={`relative w-full ${sizeClasses[size]} ${bgClass} rounded-[2.5rem] shadow-2xl animate-fade-in-up overflow-hidden`}>
                {/* Header */}
